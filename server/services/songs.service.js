...
class SongService {
  ...
  async getSongById (id) {
    const query = { id };
    const song = await this.collection.find(query).toArray();
    if (song.length === 0) {
      return null;
    } else {
      return song.pop();
  ...
  async updateSongLike (id) {
    const filter = { id };
    const status = await (await this.getSongById(id)).liked;
    const setQuery = { $set: { liked: !status } };
  ...
  async search (substring, exact) {
    let filter;
    if (exact) {
      filter = { $or: [{ name: { $regex: `${substring}` } }, { genre: { $regex: `${substring}` } }] };
    } else {
      filter = { $or: [{ name: { $regex: `${substring}`, $options: "i" } }, { genre: { $regex: `${substring}`, $options: "i" } }] };
    }
